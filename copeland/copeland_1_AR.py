'''
Runs the Copeland analysis for Average Regret
'''
...
for scenario in scenarios:
    ...
    regret = maximum_utility - experiments[:, 7].astype(np.float)
    experiments = experiments[:, (1,2,7)]
    approach_utility = np.column_stack((experiments[:,1],regret))
    ...
    with open(store_path +"\copeland_by_scenario\Copeland_R" + scenario.scenario_ID + ".txt", "w") as f:
...
with open(store_path +"\Copeland_R.txt", "w") as f:
    f.write("\n".join(" ".join(map(str, x)) for x in sorted_array))
